...
const Controls: React.FC<ControlsProps> = ({ difficulty, onDifficultyChange, onNewGame, onCheck, onHint, onSolve, isSolved }) => {
  return (
    <div className="w-full max-w-sm md:max-w-md lg:max-w-lg mt-4 flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
                ...
                <select 
                    ...
                    className="bg-slate-700 border border-slate-600 rounded-md p-2 text-white focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                >
        ...
        <div className="grid grid-cols-3 gap-2">
            <Button onClick={onCheck} disabled={isSolved}>Check</Button>
            <Button onClick={onHint} disabled={isSolved}>Hint</Button>
            <Button onClick={onSolve} disabled={isSolved} className="bg-amber-600 hover:bg-amber-500 focus:ring-amber-400">Solve</Button>
        </div>
    </div>
  );
};
